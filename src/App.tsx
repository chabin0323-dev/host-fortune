...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mike_premium_final_v9');
    if (saved) {
  ...
  const handleStartFortune = () => {
    ...
    setTimeout(() => {
      ...
      const seedVal = (year + bloodType + constellation).split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
      ...
      const getBioAtDate = (t: Date) => {
        ...
        const e = Math.round(Math.sin((2 * Math.PI * (diff + (seedVal % 7))) / 28) * 50 + 50);
        const i = Math.round(Math.sin((2 * Math.PI * (diff + (seedVal % 11))) / 33) * 50 + 50);
      ...
      setFortuneResult({
        ...
        general: getCommentByStars(mainBio.star),
        bio: mainBio,
        stars: { 
          total: mainBio.star, 
          money: Math.max(1, Math.min(5, (seedVal % 5) + 1)), 
          health: Math.max(1, Math.min(5, Math.floor(mainBio.p / 20) + 1)), 
          love: Math.max(1, Math.min(5, ((seedVal + baseDate.getDate()) % 4) + 2)), 
          work: Math.max(1, Math.min(5, Math.floor(mainBio.i / 20) + 1)) 
        },
        lucky: { direction: DIRECTIONS[seedVal % 8], number: (seedVal % 9) + 1, item: "天然石", color: "ゴールド" },
        weekly
      });
  ...
  const handleLock = () => {
    localStorage.setItem('mike_premium_final_v9', JSON.stringify({y:year, m:month, d:day, bt:bloodType, cs:constellation, zd:zodiac}));
    setIsLocked(true);
  };
  ...
  const resetAll = () => {
    ...
    setIsLocked(false);
    localStorage.removeItem('mike_premium_final_v9');
  };
        ...
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex justify-end gap-2">
              ...
              <><button onClick={() => { localStorage.removeItem('mike_premium_final_v9'); setIsLocked(false); }} className="bg-red-900/20 text-red-400 text-[10px] py-2.5 px-4 rounded-lg">解除</button>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" onClick={() => setShowManual(false)}>
          ...
          <div className="relative bg-[#111827] border border-slate-700 p-8 rounded-3xl max-w-sm w-full shadow-2xl text-left text-sm text-gray-300" onClick={e=>e.stopPropagation()}>
            ...
            <ul className="space-y-4">
              ...
              <li>2. 不明な箇所があっても、現時刻の星位から鑑定可能です。</li>
