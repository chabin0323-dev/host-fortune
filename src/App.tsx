...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mike_final_v23');
    if (saved) {
  ...
  const handleStartFortune = () => {
    ...
    setTimeout(() => {
      ...
      const getBioAtDate = (t: Date, offset: number) => {
        const diff = Math.floor((t.getTime() - birthBase.getTime()) / (24 * 60 * 60 * 1000));
        // ★①星が1〜5まで必ずバラけるように波の計算を調整
        const wave = Math.sin((2 * Math.PI * (diff + 5 + offset)) / 28) * 50 + 50;
        return Math.max(1, Math.min(5, Math.floor(wave / 20) + 1));
      };
      
      const mainStar = getBioAtDate(baseDate, totalSeed % 7);
      const dayLabels = ['日', '月', '火', '水', '木', '金', '土'];
      const weekly = Array.from({ length: 7 }, (_, idx) => {
        const d = new Date(baseDate);
        d.setDate(baseDate.getDate() + idx);
        // 曜日ごとに異なるオフセットを加えて星を分散させる
        return { date: `${d.getMonth() + 1}/${d.getDate()}`, day: dayLabels[d.getDay()], star: getBioAtDate(d, idx * 3) };
      });

      setFortuneResult({
        dateStr: `${baseDate.getFullYear()}年${(baseDate.getMonth() + 1)}月${baseDate.getDate()}日`,
        general: getDailyGeneralComment(mainStar, totalSeed),
        advice: getAdvice(mainStar, totalSeed), // ②行動指針
        stars: {
          total: mainStar,
      ...
      <div className="w-full max-w-md px-6">
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex justify-end gap-2 pt-2">{!isLocked ? <button onClick={()=>{localStorage.setItem('mike_final_v23', JSON.stringify({y:yVal,m:mVal,d:dVal,bt:btVal,cs:csVal,zd:zdVal})); setIsLocked(true);}} className="bg-[#222] text-gray-400 text-[10px] py-2 px-6 rounded-lg border border-white/5 shadow-md">入力を固定</button> : <><button onClick={()=>{localStorage.removeItem('mike_final_v23'); setIsLocked(false);}} className="bg-red-900/20 text-red-400 text-[10px] py-2 px-4 rounded-lg">解除</button><button onClick={()=>{setYVal('不明');setMVal('不明');setDVal('不明');setBtVal('不明');setCsVal('不明');setZdVal('不明');setIsLocked(false);}} className="bg-[#222] text-gray-400 text-[10px] py-2 px-4 rounded-lg border border-white/5">他人を占う</button></>}</div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10 text-left">
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10 text-left">
              ...
              <div className="grid grid-cols-2 gap-3 text-[10px]">
                <div className="bg-black/40 p-3 rounded-xl flex flex-col items-center gap-1 border border-white/5"><span className="text-gray-500">ラッキーカラー</span><span className="text-yellow-400 font-bold">{fortuneResult.lucky.color}</span></div>
                <div className="bg-black/40 p-3 rounded-xl flex flex-col items-center gap-1 border border-white/5"><span className="text-gray-500">ラッキーアイテム</span><span className="text-yellow-400 font-bold text-center">{fortuneResult.lucky.item}</span></div>
              </div>
            </div>
      ...
      {showManual && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" onClick={() => setShowManual(false)}>
          ...
          <div className="relative bg-[#111827] border border-slate-700 p-8 rounded-3xl max-w-sm w-full shadow-2xl text-left text-sm text-gray-300" onClick={e=>e.stopPropagation()}>
            ...
            <ul className="space-y-4"><li>1. 日付ごとの星位を精密に解析し、星1つから5つまで適切に変化する助言をお届けします。</li><li>2. 総合運の下に、今日すぐできる「行動指針」を新たに追加しました。</li><li>3. プルダウンの「不明」を含む全ての機能はそのまま維持されています。</li></ul>
