...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('mi_ke_premium_v7');
      if (saved) {
        const d = JSON.parse(saved);
        setYear(d.year || '不明');
        setMonth(d.month || '不明');
        setDay(d.day || '不明');
        setBloodType(d.bloodType || '不明');
        setConstellation(d.constellation || '不明');
        setZodiac(d.zodiac || '不明');
        setIsLocked(true);
      }
    } catch (e) {
  ...
  const resetForOther = () => {
    ...
    setIsLocked(false);
    localStorage.removeItem('mi_ke_premium_v7');
  };
        ...
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              ...
              <div className="space-y-2"><p className="text-blue-300 font-bold">星座</p>
                <select value={constellation} onChange={(e) => setConstellation(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white">
                  <option value="不明">不明</option>
                  {["牡羊座", "牡牛座", "双子座", "蟹座", "獅子座", "乙女座", "天秤座", "蠍座", "射手座", "山羊座", "水瓶座", "魚座"].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
            ...
            <div className="flex justify-end gap-2">
              {!isLocked ? (
                <button onClick={() => {localStorage.setItem('mi_ke_premium_v7', JSON.stringify({year, month, day, bloodType, constellation, zodiac})); setIsLocked(true);}} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-6 rounded-lg">入力を固定</button>
              ) : (
                <>
                  <button onClick={() => {localStorage.removeItem('mi_ke_premium_v7'); setIsLocked(false);}} className="bg-red-900/20 text-red-400 text-[10px] py-2.5 px-4 rounded-lg">解除</button>
                  <button onClick={resetForOther} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-4 rounded-lg">他人を占う</button>
                </>
              )}
            </div>

            <button onClick={generateFortune} className="w-full py-5 rounded-2xl font-bold text-lg bg-gradient-to-r from-fuchsia-500 to-cyan-500 shadow-xl active:scale-95">鑑定を開始する</button>
          </div>
        ) : (
          /* 結果画面 */
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10 text-left">
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10">
              ...
              <p className="text-[10px] text-gray-500 mb-6 text-center">※身体・感情・知性のバイオリズム周期です。</p>
                ...
                {fortuneResult.weekly.map((item: any, idx: number) => (
                  <div key={idx} className="bg-black/40 p-4 rounded-xl flex justify-between items-center border border-white/5">
                    ...
                    <div className="flex gap-1">
                      {[...Array(5)].map((_, i) => (
                        <span key={i} className={i < item.star ? "text-yellow-400 text-xs" : "text-gray-800 text-xs"}>★</span>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: '健康運', star: fortuneResult.stars.health, color: 'text-emerald-500', comm: "休息が運を呼びます。" },
                { label: '恋愛運', star: fortuneResult.stars.love, color: 'text-pink-500', comm: "誠実さが一番。" },
                { label: '仕事運', star: fortuneResult.stars.work, color: 'text-cyan-500', comm: "努力が実る日。" }
              ].map((u, i) => (
