...
const App: React.FC = () => {
  ...
  const renderStars = (count: number, size: string = "text-lg") => (
    <div className="flex gap-0.5 justify-center">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={i < count ? "text-yellow-400" : "text-gray-600"} style={textShadow}>★</span>
      ))}
    </div>
  );
        ...
        {loading ? (
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10">
            ...
            <div className={cardBg}>
              <h3 className={`text-lg mb-4 ${titleColor} text-center border-b border-[#1e1540] pb-3`}>今日の総合運</h3>
              <div className="flex justify-center mb-5">{renderStars(fortuneResult.stars.total, "text-xl")}</div>
              <p className={`text-[14px] ${textColor} leading-relaxed font-light mb-4`}>{fortuneResult.general}</p>
              
              <div className="bg-[#1e1540]/50 p-4 rounded-xl border border-cyan-900">
                <p className="text-[10px] text-cyan-300 font-bold uppercase mb-1 tracking-wider">💡 行動指針</p>
                <p className={`text-[12px] ${textColor} leading-relaxed font-medium`}>{fortuneResult.advice}</p>
              </div>
            </div>
            
            <div className={cardBg}>
              <h3 className={`text-sm mb-6 text-center border-b border-[#1e1540] pb-2 ${titleColor}`}>週間バイオリズム</h3>
              <div className="space-y-3">
                {fortuneResult.weekly.map((w: any, i: number) => (
                  <div key={i} className={`flex items-center justify-between p-3 rounded-xl ${i === 0 ? 'bg-cyan-950/50 border border-cyan-700' : 'bg-[#0a0518]/50 border border-[#1e1540]'}`}>
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-bold w-10 ${i === 0 ? 'text-cyan-300' : 'text-gray-500'}`}>{w.date}</span>
                      <span className={`text-xs w-4 ${i === 0 ? 'text-gray-100' : 'text-gray-400'}`}>{w.day}</span>
                    </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {[ { label: '金運', star: fortuneResult.stars.money.s, color: 'text-yellow-400', comm: fortuneResult.stars.money.c }, { label: '健康運', star: fortuneResult.stars.health.s, color: 'text-emerald-400', comm: fortuneResult.stars.health.c }, { label: '恋愛運', star: fortuneResult.stars.love.s, color: 'text-pink-400', comm: fortuneResult.stars.love.c }, { label: '仕事運', star: fortuneResult.stars.work.s, color: 'text-cyan-400', comm: fortuneResult.stars.work.c } ].map((u, i) => (
                <div key={i} className={cardBg}>
                  ...
                  <p className={`text-[12px] ${textColor} leading-relaxed font-light`}>{u.comm}</p>
                </div>
              ))}
            </div>
            
            <div className={`${cardBg} text-left`}>
              <h3 className={`text-sm mb-4 text-center border-b border-[#1e1540] pb-2 ${titleColor}`}>本日のラッキー指針</h3>
              <div className="grid grid-cols-2 gap-3 text-[10px]">
                <div className="bg-[#0a0518]/50 p-3 rounded-xl flex flex-col items-center gap-1 border border-[#1e1540]">
                  <span className="text-gray-500">ラッキーカラー</span>
                  <span className={`${textColor} font-bold text-center`}>{fortuneResult.lucky.color}</span>
                </div>
                <div className="bg-[#0a0518]/50 p-3 rounded-xl flex flex-col items-center gap-1 border border-[#1e1540]">
                  <span className="text-gray-500">ラッキーアイテム</span>
                  <span className={`${textColor} font-bold text-center`}>{fortuneResult.lucky.item}</span>
                </div>
              </div>
            </div>
