...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mike_premium_ultimate_final');
    if (saved) {
      try {
  ...
  const getDynamicComment = (stars: number, totalSeed: number) => {
    const comments = {
      ...
      default: ["今日は静かに自分を見つめ直すべき充電の時です。無理に動くよりも、足元を固め、次なる飛躍の準備を整えることで運気が好転します。", "浄化の時。不要なものを手放し、心に空白を作ることで、新しい運気の風が吹き込んできます。"]
    };
    const list = stars >= 5 ? comments[5] : stars === 4 ? comments[4] : stars === 3 ? comments[3] : comments.default;
    return list[totalSeed % list.length];
  };
  ...
  const handleStartFortune = () => {
    ...
    setTimeout(() => {
      ...
      const curveData = Array.from({ length: 7 }, (_, idx) => {
        ...
        return { label: idx === 3 ? "今日" : `${d.getMonth() + 1}/${d.getDate()}`, val: b.total };
      });
  ...
  const handleLock = () => {
    localStorage.setItem('mike_premium_ultimate_final', JSON.stringify({y:yVal, m:mVal, d:dVal, bt:btVal, cs:csVal, zd:zdVal}));
    setIsLocked(true);
  };
  ...
  const resetAll = () => {
    ...
    setIsLocked(false);
    localStorage.removeItem('mike_premium_ultimate_final');
  };
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              ...
              <div className="space-y-2 flex flex-col justify-end text-center"><p>月</p>
                <select value={mVal} onChange={(e)=>setMVal(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white appearance-none">
              ...
              <div className="space-y-2 flex flex-col justify-end text-center"><p>日</p>
                <select value={dVal} onChange={(e)=>setDVal(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white appearance-none">
            ...
            <div className="flex justify-end gap-2 pt-2">
              {!isLocked ? <button onClick={handleLock} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-6 rounded-lg border border-white/5">入力を固定</button> :
              <><button onClick={() => { localStorage.removeItem('mike_premium_ultimate_final'); setIsLocked(false); }} className="bg-red-900/20 text-red-400 text-[10px] py-2.5 px-4 rounded-lg">解除</button><button onClick={resetAll} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-4 rounded-lg border border-white/5">他人を占う</button></>}
            </div>

            <button onClick={handleStartFortune} className="w-full py-5 rounded-2xl font-bold text-lg bg-gradient-to-r from-fuchsia-500 to-cyan-500 shadow-lg active:scale-95 transition-all">鑑定を開始する</button>
          </div>
        ) : (
          /* 結果画面 */
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10 text-left">
            <div className="bg-[#111] border border-white/10 p-5 rounded-xl text-center shadow-inner"><p className="text-cyan-400 text-[10px] mb-1 font-bold uppercase tracking-widest text-xs">鑑定日</p><p className="text-xl font-bold text-white tracking-widest">{fortuneResult.dateStr}</p></div>
            <div className="bg-[#1e1e1e] p-7 rounded-3xl border border-gray-800 shadow-xl"><h3 className="text-lg font-bold mb-4 text-white text-center border-b border-gray-800 pb-3">今日の総合運</h3><div className="flex justify-center mb-5">{renderStars(fortuneResult.stars.total)}</div><p className="text-[15px] text-gray-300 leading-relaxed font-light text-left">{fortuneResult.general}</p></div>
            
            {/* 運命の波形グラフ */}
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10">
              <h3 className="text-sm font-bold text-white mb-2 text-center">精密バイオリズム曲線</h3>
              <p className="text-[10px] text-gray-500 mb-6 text-center leading-relaxed">※過去3日〜未来3日の運勢の波を視覚化しています。</p>
              <div className="w-full h-32 flex items-end justify-between px-2 gap-1 relative border-b border-gray-800">
                ...
                <div className="absolute top-1/2 left-0 right-0 h-px bg-gray-900"></div>
                {fortuneResult.curveData.map((d:any, i:number) => (
                  <div key={i} className="flex-1 flex flex-col items-center">
                    <div className={`w-2.5 bg-gradient-to-t ${i === 3 ? 'from-cyan-500 to-fuchsia-500' : 'from-gray-800 to-gray-600'} rounded-t-sm relative transition-all duration-1000`} style={{ height: `${d.val}%` }}></div>
                    <p className={`mt-3 text-[9px] ${i === 3 ? 'text-white font-bold' : 'text-gray-600'} font-mono uppercase`}>{d.label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10">
              ...
              <div className="mt-4 space-y-3 text-xs">
                <div className="bg-black/40 p-3 rounded-xl flex justify-between px-4 border border-white/5"><span className="text-gray-500">アイテム:</span><span className="text-white font-bold">{fortuneResult.lucky.item}</span></div>
                <div className="bg-black/40 p-3 rounded-xl flex justify-between px-4 border border-white/5"><span className="text-gray-500">カラー:</span><span className="text-white font-bold">{fortuneResult.lucky.color}</span></div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 text-left">
              {[{ label: '金運', star: fortuneResult.stars.money, color: 'text-yellow-500', comm: "必要なものを見極めて。" }, { label: '健康運', star: fortuneResult.stars.health, color: 'text-emerald-500', comm: "適度な休息が運を呼ぶ。" }, { label: '恋愛運', star: fortuneResult.stars.love, color: 'text-pink-500', comm: "誠実な対話が鍵です。" }, { label: '仕事運', star: fortuneResult.stars.work, color: 'text-cyan-500', comm: "努力が実を結ぶ日。" }].map((u, i) => (
      ...
      {showManual && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" onClick={() => setShowManual(false)}>
          ...
          <div className="relative bg-[#111827] border border-slate-700 p-8 rounded-3xl max-w-sm w-full shadow-2xl text-left text-sm text-gray-300" onClick={e=>e.stopPropagation()}>
            ...
            <ul className="space-y-4"><li>1. 情報を入力し「鑑定を開始」をタップ。</li><li>2. 不明な箇所があっても、鑑定可能です。</li><li>3. 波形グラフで運勢の『勢い』を読み解きます。</li></ul>
