...
// ★可読性を確保するためのCSSクラスを定義
const textShadow = { textShadow: '0 1px 3px rgba(0,0,0,0.8)' };
const cardBg = "bg-white/85 backdrop-blur-sm shadow-xl"; // コンテンツの背景：少し透ける白
...
const App: React.FC = () => {
  ...
  const renderStars = (count: number, size: string = "text-lg") => (
    <div className="flex gap-0.5 justify-center">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={i < count ? "text-yellow-500" : "text-gray-300"} style={textShadow}>★</span>
      ))}
    </div>
  );

  return (
    // ★鑑定結果画面のみ神秘的な背景を適用。文字の可読性を確保するため全体を黒で覆い、その上にコンテンツを載せる
    <div className={`min-h-screen ${page === 'RESULT' ? 'bg-[#050010]' : 'bg-black'} text-gray-900 font-sans flex flex-col items-center pb-20 relative text-center relative`}>
      
      {/* 鑑定結果画面のみ背景画像を表示 */}
      {page === 'RESULT' && (
        <div className="absolute inset-0 z-0 opacity-40 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1518709766631-a6a7f4593b3e?q=80&w=2000&auto=format&fit=crop)' }}></div>
      )}
      ...
      <div className="mt-8 mb-10 flex items-center justify-center gap-3 relative z-10">
        <div className="flex items-center gap-1 text-3xl font-bold" style={page === 'RESULT' ? { color: '#fff', textShadow: '0 2px 10px rgba(100,255,255,0.5)' } : { color: '#fff' }}>
      ...
      <div className="w-full max-w-md px-6 relative z-10">
        {loading ? (
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10 text-left">
            <div className={`p-5 rounded-xl text-center shadow-inner ${cardBg}`}><p className="text-cyan-700 text-[10px] font-bold uppercase tracking-widest">鑑定日：{fortuneResult.dateStr}</p></div>
            
            <div className={`p-7 rounded-3xl ${cardBg}`}>
              <h3 className="text-lg font-bold mb-4 text-gray-950 text-center border-b border-gray-200 pb-3">今日の総合運</h3>
              <div className="flex justify-center mb-5">{renderStars(fortuneResult.stars.total, "text-xl")}</div>
              <p className="text-[14px] text-gray-800 leading-relaxed font-light mb-4">{fortuneResult.general}</p>
              <div className="bg-cyan-50 p-4 rounded-xl border border-cyan-100">
                <p className="text-[10px] text-cyan-800 font-bold uppercase mb-1 tracking-wider">💡 行動指針</p>
                <p className="text-[12px] text-gray-900 leading-relaxed font-medium">{fortuneResult.advice}</p>
              </div>
            </div>
            
            <div className={`p-6 rounded-2xl ${cardBg}`}><h3 className="text-sm font-bold text-gray-950 mb-6 text-center border-b border-gray-200 pb-2">週間バイオリズム</h3>
              <div className="space-y-3">{fortuneResult.weekly.map((w: any, i: number) => (<div key={i} className={`flex items-center justify-between p-3 rounded-xl ${i === 0 ? 'bg-cyan-100 border border-cyan-200' : 'bg-gray-50'}`}><div className="flex items-center gap-3"><span className={`text-[10px] font-bold w-10 ${i === 0 ? 'text-cyan-800' : 'text-gray-500'}`}>{w.date}</span><span className={`text-xs w-4 ${i === 0 ? 'text-gray-900' : 'text-gray-600'}`}>{w.day}</span></div><div>{renderStars(w.star, "text-sm")}</div><span className="text-[9px] text-gray-500 uppercase">{i === 0 ? 'Today' : ''}</span></div>))}</div>
            </div>
            
            <div className="grid grid-cols-1 gap-4">{[ { label: '金運', star: fortuneResult.stars.money.s, color: 'text-yellow-600', comm: fortuneResult.stars.money.c }, { label: '健康運', star: fortuneResult.stars.health.s, color: 'text-emerald-700', comm: fortuneResult.stars.health.c }, { label: '恋愛運', star: fortuneResult.stars.love.s, color: 'text-pink-700', comm: fortuneResult.stars.love.c }, { label: '仕事運', star: fortuneResult.stars.work.s, color: 'text-cyan-700', comm: fortuneResult.stars.work.c } ].map((u, i) => (
                <div key={i} className={`p-5 rounded-2xl ${cardBg}`}><div className="flex justify-between items-center mb-3"><p className={`font-bold text-sm ${u.color}`}>{u.label}</p><div className="flex gap-0.5">{renderStars(u.star, "text-sm")}</div></div><p className="text-[12px] text-gray-800 leading-relaxed font-light">{u.comm}</p></div>
              ))}
            </div>
            
            <div className={`p-6 rounded-2xl ${cardBg} text-left`}><h3 className="text-sm font-bold text-gray-950 mb-4 text-center">本日のラッキー指針</h3><div className="grid grid-cols-2 gap-3 text-[10px]"><div className="bg-gray-50 p-3 rounded-xl flex flex-col items-center gap-1 border border-gray-100"><span className="text-gray-500">ラッキーカラー</span><span className="text-gray-950 font-bold">{fortuneResult.lucky.color}</span></div><div className="bg-gray-50 p-3 rounded-xl flex flex-col items-center gap-1 border border-gray-100"><span className="text-gray-500">ラッキーアイテム</span><span className="text-gray-950 font-bold text-center">{fortuneResult.lucky.item}</span></div></div></div>
            <button onClick={() => setPage('INPUT')} className="w-full py-4 rounded-xl bg-white/10 border border-white/20 text-white text-xs tracking-widest mt-6">戻る</button>
          </div>
        )}
      </div>
      ...
      {showManual && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" onClick={() => setShowManual(false)}>
          ...
          <div className="relative bg-[#111827] border border-slate-700 p-8 rounded-3xl max-w-sm w-full shadow-2xl text-left text-sm text-gray-300" onClick={e=>e.stopPropagation()}>
            ...
            <ul className="space-y-4"><li>1. どれか一つでも情報を入力すれば、宇宙の波動をキャッチして鑑定可能です。</li><li>2. 「行動指針」は、今日のあなたの運気をさらに高める具体的なアクションです。</li><li>3. 鑑定結果画面の背景は神秘的な雰囲気ですが、文字の可読性を最優先に設計されています。</li></ul>
