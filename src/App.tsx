...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mike_premium_v19');
    if (saved) {
  ...
  const getLongGeneralComment = (stars: number) => {
    ...
    return comments[stars] || "穏やかな運気の中にいます。自分を信じて。";
  };
      ...
      <div className="w-full max-w-md px-6">
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex justify-end gap-2 pt-2">{!isLocked ? <button onClick={()=>{localStorage.setItem('mike_premium_v19', JSON.stringify({y:yVal,m:mVal,d:dVal,bt:btVal,cs:csVal,zd:zdVal})); setIsLocked(true);}} className="bg-[#222] text-gray-400 text-[10px] py-2 px-6 rounded-lg border border-white/5 shadow-md">入力を固定</button> : <><button onClick={()=>{localStorage.removeItem('mike_premium_v19'); setIsLocked(false);}} className="bg-red-900/20 text-red-400 text-[10px] py-2 px-4 rounded-lg">解除</button><button onClick={()=>{setYVal('不明');setMVal('不明');setDVal('不明');setBtVal('不明');setCsVal('不明');setZdVal('不明');setIsLocked(false);}} className="bg-[#222] text-gray-400 text-[10px] py-2 px-4 rounded-lg border border-white/5">他人を占う</button></>}</div>
