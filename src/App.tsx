...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('mike_ultimate_v10');
    if (saved) {
      const d = JSON.parse(saved);
      setYear(d.y); setMonth(d.m); setDay(d.d);
      setBloodType(d.bt); setConstellation(d.cs); setZodiac(d.zd);
      setIsLocked(true);
    }
  }, []);
  ...
  const getDynamicComment = (stars: number, dateSeed: number) => {
    const comments = {
      5: [
        "宇宙の波動が最高潮に達しています。これまでの努力が黄金の結果を連れてくる日。迷わず最大の決断を。",
        "魂の輝きが周囲を圧倒します。あなたが主役となり、運命の歯車を大きく動かすチャンスが到来しました。"
      ],
      4: [
        "運気は高水準で安定。誠実な対話が信頼を深め、滞っていた計画が音を立てて動き始めます。自信を持って。",
        "知性と直感のバランスが完璧です。新しい情報の波に飛び込むことで、人生のパラダイムシフトが起こります。"
      ],
      3: [
        "穏やかで心地よい一日です。無理に攻めるよりも、足元を固め、周囲との調和を楽しむことで開運します。",
        "充電に適した運気。日常の小さな幸せを大切に。夜、自分へのご褒美を用意すると運気がさらに向上します。"
      ],
      default: [
        "今日は静寂を友にしてください。外へ向かうよりも内面を磨くことで、次なる飛躍の種が芽生えます。",
        "浄化の時。不要なものを手放し、心に空白を作ることで、新しい運気の風が吹き込んできます。"
      ]
    };
  ...
  const handleStartFortune = () => {
    ...
    setTimeout(() => {
      ...
      // 個人シード
      const pSeed = (year + bloodType + constellation + zodiac).split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
      ...
      const getBioAtDate = (t: Date) => {
        ...
        return { p, e, i, star: Math.max(1, Math.min(5, Math.floor(e / 20) + 1)) };
      };
  ...
  const resetAll = () => {
    ...
    setIsLocked(false);
    localStorage.removeItem('mike_ultimate_v10');
  };
      ...
      <div className="w-full max-w-md px-6">
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="space-y-2"><p className="text-blue-300 font-bold">生年月日</p><p>年</p>
                <select value={year} onChange={(e)=>setYear(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none">
                  <option value="不明">不明</option>{Array.from({ length: 80 }, (_, i) => (2026 - i).toString()).map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div className="space-y-2 flex flex-col justify-end"><p>月</p>
                <select value={month} onChange={(e)=>setMonth(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none">
                  <option value="不明">不明</option>{Array.from({ length: 12 }, (_, i) => (i + 1).toString()).map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div className="space-y-2 flex flex-col justify-end"><p>日</p>
                <select value={day} onChange={(e)=>setDay(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none">
                  <option value="不明">不明</option>{Array.from({ length: 31 }, (_, i) => (i + 1).toString()).map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="space-y-2"><p className="text-blue-300 font-bold">血液型</p><select value={bloodType} onChange={(e)=>setBloodType(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none"><option value="不明">不明</option><option value="A型">A型</option><option value="B型">B型</option><option value="O型">O型</option><option value="AB型">AB型</option></select></div>
              <div className="space-y-2 text-center"><p className="text-blue-300 font-bold">星座</p><select value={constellation} onChange={(e)=>setConstellation(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none"><option value="不明">不明</option>{["牡羊座", "牡牛座", "双子座", "蟹座", "獅子座", "乙女座", "天秤座", "蠍座", "射手座", "山羊座", "水瓶座", "魚座"].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
              <div className="space-y-2 text-center"><p className="text-blue-300 font-bold">干支</p><select value={zodiac} onChange={(e)=>setZodiac(e.target.value)} disabled={isLocked} className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white outline-none"><option value="不明">不明</option>{["子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              {!isLocked ? <button onClick={() => {localStorage.setItem('mike_ultimate_v10', JSON.stringify({y:year, m:month, d:day, bt:bloodType, cs:constellation, zd:zodiac})); setIsLocked(true);}} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-6 rounded-lg border border-white/5 shadow-md">入力を固定</button> :
              <><button onClick={() => { localStorage.removeItem('mike_ultimate_v10'); setIsLocked(false); }} className="bg-red-900/20 text-red-400 text-[10px] py-2.5 px-4 rounded-lg">解除</button><button onClick={resetAll} className="bg-[#222] text-gray-400 text-[10px] py-2.5 px-4 rounded-lg border border-white/5 shadow-md">他人を占う</button></>}
            </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10 text-left">
            <div className="bg-[#111] border border-white/10 p-5 rounded-xl text-center"><p className="text-cyan-400 text-[10px] mb-1 font-bold uppercase tracking-widest">鑑定日</p><p className="text-xl font-bold text-white tracking-widest">{fortuneResult.dateStr}</p></div>
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10"><h3 className="text-sm font-bold text-white mb-2 text-center">精密バイオリズム解析</h3><p className="text-[10px] text-gray-500 mb-6 text-center">※身体・感情・知性の24時間周期を可視化しています。</p>
              <div className="space-y-5">
                {[{ label: '身体', val: fortuneResult.bio.p, color: 'bg-emerald-500' }, { label: '感情', val: fortuneResult.bio.e, color: 'bg-pink-500' }, { label: '知性', val: fortuneResult.bio.i, color: 'bg-cyan-500' }].map((item, idx) => (
                  <div key={idx} className="space-y-1.5 px-2 text-left"><div className="flex justify-between text-[10px] text-gray-400"><span>{item.label}</span><span>{item.val}%</span></div><div className="h-1.5 w-full bg-black rounded-full overflow-hidden"><div className={`h-full ${item.color} transition-all duration-1000`} style={{ width: `${item.val}%` }}></div></div></div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#111] p-6 rounded-2xl border border-white/10"><h3 className="text-sm font-bold text-white mb-6 text-center">本日のラッキー指針</h3>
              <div className="grid grid-cols-2 gap-4"><div className="bg-black/40 p-4 rounded-xl border border-white/5 flex flex-col items-center"><span className="text-[10px] text-emerald-400 font-bold mb-1">吉方位</span><span className="text-lg font-bold text-white">{fortuneResult.lucky.direction}</span></div><div className="bg-black/40 p-4 rounded-xl border border-white/5 flex flex-col items-center text-fuchsia-400"><span className="text-[10px] font-bold mb-1">数</span><span className="text-lg font-bold text-white">{fortuneResult.lucky.number}</span></div></div>
              <div className="mt-4 space-y-3 text-xs"><div className="bg-black/40 p-3 rounded-xl flex justify-between px-4"><span className="text-gray-500">アイテム:</span><span className="font-bold text-white">{fortuneResult.lucky.item}</span></div><div className="bg-black/40 p-3 rounded-xl flex justify-between px-4"><span className="text-gray-500">カラー:</span><span className="font-bold text-white">{fortuneResult.lucky.color}</span></div></div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-left">
              {[{ label: '金運', star: fortuneResult.stars.money, color: 'text-yellow-500', comm: "好機を見極めて。" }, { label: '健康運', star: fortuneResult.stars.health, color: 'text-emerald-500', comm: "休息が運を呼ぶ。" }, { label: '恋愛運', star: fortuneResult.stars.love, color: 'text-pink-500', comm: "誠実な対話が鍵。" }, { label: '仕事運', star: fortuneResult.stars.work, color: 'text-cyan-500', comm: "努力が実を結ぶ日。" }].map((u, i) => (
                <div key={i} className="bg-[#1e1e1e] p-4 rounded-2xl border border-gray-800"><p className={`font-bold mb-2 text-xs ${u.color}`}>{u.label}</p><div className="mb-2">{renderStars(u.star)}</div><p className="text-[10px] text-gray-500 leading-tight">{u.comm}</p></div>
              ))}
            </div>
      ...
      {showManual && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" onClick={() => setShowManual(false)}>
          ...
          <div className="relative bg-[#111827] border border-slate-700 p-8 rounded-3xl max-w-sm w-full shadow-2xl text-left text-sm text-gray-300" onClick={e=>e.stopPropagation()}>
            ...
            <ul className="space-y-4"><li>1. 情報を入力し「鑑定を開始」をタップ。</li><li>2. 生年月日などが不明でも鑑定可能です。</li><li>3. 星の周期を精密に解析し、今日の指針を導き出します。</li></ul>
