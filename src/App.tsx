...
// ★デザイン・視認性設定
const textShadow = { textShadow: '0 2px 4px rgba(0,0,0,0.8)' };
const cardBg = "bg-[#0a0518]/90 backdrop-blur-md border border-indigo-500/20 rounded-3xl p-6 shadow-2xl text-left";
const titleColor = "text-cyan-300 font-bold";
const textColor = "text-indigo-50";

// ★【修正】黒画面を回避し、確実に神秘的な星空を描写するスタイル
const mysticBackground = {
  backgroundColor: '#050212',
  backgroundImage: `
    radial-gradient(circle at 50% 10%, #1b0a3d 0%, transparent 50%),
    radial-gradient(circle at 10% 90%, #0a1b3d 0%, transparent 50%),
  ...
  `,
  backgroundSize: '100% 100%, 100% 100%, 200px 200px, 200px 200px, 200px 200px, 200px 200px, 200px 200px, 200px 200px',
...
const App: React.FC = () => {
  ...
  const renderStars = (count: number, size: string = "text-lg") => (
    <div className="flex gap-0.5 justify-center">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={i < count ? "text-yellow-400" : "text-indigo-900"} style={textShadow}>★</span>
      ))}
    </div>
  );

  return (
    <div style={mysticBackground} className="min-h-screen font-sans flex flex-col items-center pb-20 relative text-center overflow-x-hidden">
        ...
        {loading ? (
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="space-y-2"><p className="text-indigo-300 font-bold">生年月日</p><select value={yVal} onChange={(e)=>setYVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white outline-none appearance-none"><option value="不明">不明</option>{Array.from({ length: 80 }, (_, i) => (2026 - i).toString()).map(v => <option key={v} value={v}>{v}年</option>)}</select></div>
              <div className="space-y-2 flex flex-col justify-end"><select value={mVal} onChange={(e)=>setMVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white outline-none appearance-none"><option value="不明">不明</option>{Array.from({ length: 12 }, (_, i) => (i + 1).toString()).map(v => <option key={v} value={v}>{v}月</option>)}</select></div>
              <div className="space-y-2 flex flex-col justify-end"><select value={dVal} onChange={(e)=>setDVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white outline-none appearance-none"><option value="不明">不明</option>{Array.from({ length: 31 }, (_, i) => (i + 1).toString()).map(v => <option key={v} value={v}>{v}日</option>)}</select></div>
            </div>
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="space-y-2"><p className="text-indigo-300 font-bold">血液型</p><select value={btVal} onChange={(e)=>setBtVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white appearance-none"><option value="不明">不明</option>{["A型","B型","O型","AB型"].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
              <div className="space-y-2 text-center"><p className="text-indigo-300 font-bold">星座</p><select value={csVal} onChange={(e)=>setCsVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white appearance-none"><option value="不明">不明</option>{["牡羊座", "牡牛座", "双子座", "蟹座", "獅子座", "乙女座", "天秤座", "蠍座", "射手座", "山羊座", "水瓶座", "魚座"].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
              <div className="space-y-2 text-center"><p className="text-indigo-300 font-bold">干支</p><select value={zdVal} onChange={(e)=>setZdVal(e.target.value)} disabled={isLocked} className="w-full bg-[#0a0518] border border-indigo-500/30 rounded-lg p-3 text-white appearance-none"><option value="不明">不明</option>{["子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥"].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
            </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-10 pb-10">
            ...
            <div className={cardBg}>
              ...
              <div className="bg-indigo-950/50 p-4 rounded-xl border border-indigo-500/30"><p className="text-[10px] text-cyan-300 font-bold uppercase mb-1">💡 行動指針</p><p className="text-[12px] text-white font-medium">{fortuneResult.advice}</p></div>
            </div>
            <div className={cardBg}>
                ...
                {fortuneResult.weekly.map((w: any, i: number) => (
                  <div key={i} className={`flex items-center justify-between p-3 rounded-xl ${i === 0 ? 'bg-indigo-900/40 border border-indigo-500/40' : 'bg-black/20'}`}>
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-bold w-10 ${i === 0 ? 'text-indigo-300' : 'text-indigo-400'}`}>{w.date}</span>
            ...
            <div className="grid grid-cols-1 gap-4">
              {[ { label: '金運', star: fortuneResult.stars.money.s, color: 'text-yellow-400', comm: fortuneResult.stars.money.c }, { label: '健康運', star: fortuneResult.stars.health.s, color: 'text-emerald-400', comm: fortuneResult.stars.health.c }, { label: '恋愛運', star: fortuneResult.stars.love.s, color: 'text-pink-400', comm: fortuneResult.stars.love.c }, { label: '仕事運', star: fortuneResult.stars.work.s, color: 'text-cyan-400', comm: fortuneResult.stars.work.c } ].map((u, i) => (
                <div key={i} className={cardBg}>
                  ...
                  <p className={`text-[12px] leading-relaxed font-light ${textColor}`}>{u.comm}</p>
                </div>
              ))}
            </div>
